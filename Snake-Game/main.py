...
def next_tern(snale, food):
    ...
    if check_collisions(snake):
        ...
        window.after(SPEED, next_tern, snake, food)
...
wimdow_width = window.winfo_width()
window_height = window.winfo_height()
screen_width = window.winfo_screenwidth()
screen_height = window.winfo_screenheight()

x = int((screen_width/2) - (wimdow_width/2))
y = int((screen_height/2) - (window_height/2))


window.geometry(f"{wimdow_width}x{window_height}+{x}+{y}" )
